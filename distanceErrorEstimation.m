...
function distanceErrorEstimation(LED, RIS1, RIS2, RIS3, RIS4, number_of_plot_needed, ...
    p, alpha, beta, Phi_FoV, a, rho, Psi, A_pd, T_of, R_pd, q_0, k_B, T_k, eta, I_2, I_3, Gamma, g_m, I_bg, G_0, B, ...
    x_probe, y_probe, z_plot_index, number_of_samples)
...
received_power_RIS1_array   = zeros(1, number_of_samples);
    ...
    for i=1:length(x_probe)
        for j=1:length(y_probe)
            ...
            for sampling_idx = 1:number_of_samples
                ...
                %received_power_LED_array(sampling_idx) = (R_pd/A_pd)*p*singleEntityContribution(LED, 0, PDect_pos_LED, alpha, beta, Phi_FoV, a, rho, Psi, A_pd, T_of, 1);
                received_power_LED_array(sampling_idx) = R_pd*p*singleEntityContribution(LED, 0, PDect_pos_LED, alpha, beta, Phi_FoV, a, rho, Psi, A_pd, T_of, 1);
    ...
    for i=1:length(x_probe)
        for j=1:length(y_probe)
            ...
            for sampling_idx = 1:number_of_samples
                ...
                %received_power_RIS1_array(sampling_idx) = (R_pd/A_pd)*p*singleEntityContribution(LED, RIS1, PDect_pos_RIS1, alpha, beta, Phi_FoV, a, rho, Psi, A_pd, T_of, 0);
                received_power_RIS1_array(sampling_idx) = R_pd*p*singleEntityContribution(LED, RIS1, PDect_pos_RIS1, alpha, beta, Phi_FoV, a, rho, Psi, A_pd, T_of, 0);
